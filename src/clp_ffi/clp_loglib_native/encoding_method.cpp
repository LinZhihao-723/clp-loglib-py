...
namespace clp_loglib_native::encoder {
    PyObject* encode_preamble (PyObject* self, PyObject* args) {
        ffi::epoch_time_ms_t ref_timestamp;
        const char* input_timestamp_format;
        const char* input_timezone;

        if (!PyArg_ParseTuple(args, "Lss", &ref_timestamp, &input_timestamp_format,
            &input_timezone))
        {
        ...
        if (!ffi::ir_stream::four_byte_encoding::encode_preamble(timestamp_format,
            timestamp_pattern_syntax, timezone, ref_timestamp, ir_buf))
        {
            PyErr_SetString(PyExc_RuntimeError,
        ...
        PyObject* output_bytearray = PyByteArray_FromStringAndSize(
            reinterpret_cast<char*>(ir_buf.data()), 
            ir_buf.size()
        );
    ...
    PyObject* encode_message_and_timestamp (PyObject* self, PyObject* args) {
        ...
        if (!PyArg_ParseTuple(args, "Ly*", &ref_timestamp, &input_buffer)) {
        ...
        std::string_view msg(static_cast<const char*>(input_buffer.buf), input_buffer.len);
        const auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::high_resolution_clock::now().time_since_epoch()
        ).count();
        const ffi::epoch_time_ms_t current_timestamp = static_cast<ffi::epoch_time_ms_t>(ms);
        const auto delta = current_timestamp - ref_timestamp;

        // Encode the message
        if (!ffi::ir_stream::four_byte_encoding::encode_message(delta, msg, logtype, ir_buf)) {
            PyBuffer_Release(&input_buffer);
            PyErr_SetString(PyExc_RuntimeError,
                "Native message encoder failed to encode the message.");
            Py_RETURN_NONE;
        }
    ...
    PyObject* encode_message (PyObject* self, PyObject* args) {
        ...
        std::string_view msg(static_cast<const char*>(input_buffer.buf), input_buffer.len);
        if (!ffi::ir_stream::four_byte_encoding::encode_message(msg, logtype, ir_buf)) {
    ...
    PyObject* encode_timestamp (PyObject* self, PyObject* args) {
        ffi::epoch_time_ms_t ref_timestamp;
        if (!PyArg_ParseTuple(args, "L", &ref_timestamp)) {
            PyErr_SetString(PyExc_RuntimeError, "Native encoder failed to parse Python arguments.");
            Py_RETURN_NONE;
        }
        ...
        std::vector<int8_t> ir_buf;
        if (!ffi::ir_stream::four_byte_encoding::encode_timestamp(delta, ir_buf)) {
            PyErr_SetString(PyExc_RuntimeError,
                "Native encoder failed to encode the timestamp delta.");
            Py_RETURN_NONE;
        }
